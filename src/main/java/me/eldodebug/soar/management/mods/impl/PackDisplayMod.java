...
public class PackDisplayMod extends HUDMod {
	...
	private void loadTexture() {
		...
		try {
			...
			try {
				dynamicTexture = new DynamicTexture(mc.getResourcePackRepository().getRepositoryEntries().get(0).getResourcePack().getPackImage());
	...
	private IResourcePack getCurrentPack() {
		...
		return mc.getResourcePackRepository().getRepositoryEntries().get(0).getResourcePack();
	}
}
